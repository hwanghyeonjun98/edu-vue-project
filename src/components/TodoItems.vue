	...
	<div class="card mb-2" v-for="(todo, idx) in todos" :key="todo.id">
		<div class="card-body p-2 d-flex justify-content-between align-items-center" @click="moveToPage(todo.id)">
			<div class="form-check">
				<input type="checkbox" class="form-check-input" :id="'todo_'+todo.id" :checked="todo.complated" :name="'todo_'+todo.id" @change="toggleTodo(idx, $event)" @click.stop />
				<!-- <label :for="'todo_'+todo.id" :style="todo.complated ? todoSyle : {}" class="form-check-label">{{ todo.subject }}</label> -->
				<label class="form-check-label" :class="{todo: todo.complated}" :for="'todo_'+todo.id" @click.stop>{{ todo.subject }}</label>
			</div>
